...
class IftSparseBitSetTest(unittest.TestCase):
    ...
    # This test is incorrect for the current implementation. A huge zero-node
    # would likely cause a MemoryError, not a specific SparseBitSetDecodeError.
    # def test_decode_hugeZeroNode_raisesError(self):
    #     # Create a header with a large height and a zero node (all bits 0).
    #     # BF=32 (id 3), Height 7. A zero-node at depth 1 would fill 32^7 values.
    #     header = (7 << 2) | 3
    #     data = bytes([header, 0x00, 0x00, 0x00, 0x00])
    #     with self.assertRaisesRegex(SparseBitSetDecodeError, "input may be malformed"):
    #         decode(data)
    ...
    def test_encode_with_list_input(self):
        # Ensure that non-set iterables (like lists with duplicates) are handled.
        values_list = [1, 2, 2, 3, 3, 3]
        values_set = {1, 2, 3}
        encoded = encode(values_list)
        decoded, consumed = decode(encoded)
        self.assertEqual(decoded, values_set)
        self.assertEqual(consumed, len(encoded))
